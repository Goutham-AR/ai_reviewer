...
async function main() {
    ...
    app.post("/review", async (req, res, next) => await reviewController.doReviewController(req, res, next));
    app.post("/re-review", async (req, res, next) => await reviewController.doReReviewController(req, res, next));

    app.use(errorMiddleware);

    // Not found route
    app.use((_req, res, _next) => {
        return res.status(StatusCodes.NOT_FOUND).json({ message: "Requested route does not exist" });
    })
